...
const ConfigSchema = z.object({
  ...
  OIDC_OWNER_GROUP: z.string().default("aialra-kimi-owner"),
  OIDC_ELEVATION_ISSUER: z.string().url().optional(),
  OIDC_ELEVATION_CLIENT_ID: z.string().optional(),
  OIDC_ELEVATION_CLIENT_SECRET: z.string().optional(),
  OIDC_ELEVATION_CLIENT_SECRET_FILE: z.string().optional(),
  OIDC_ELEVATION_REDIRECT_URI: z.string().url().optional(),
...
export function loadConfig(input: NodeJS.ProcessEnv = process.env): AppConfig {
  ...
  const elevationOidcClientSecret = secretValue(
    "OIDC_ELEVATION_CLIENT_SECRET",
    env.OIDC_ELEVATION_CLIENT_SECRET,
    env.OIDC_ELEVATION_CLIENT_SECRET_FILE,
  );
  const databaseKey =
  ...
  const elevationOidcValues = [
    env.OIDC_ELEVATION_CLIENT_ID,
    elevationOidcClientSecret,
    env.OIDC_ELEVATION_REDIRECT_URI,
  ];
  const hasPartialElevationOidc =
    elevationOidcValues.some(Boolean) && !elevationOidcValues.every(Boolean);
  if (hasPartialElevationOidc)
    throw new Error("Elevation OIDC configuration must be complete");
  if (
    elevationOidcValues.every(Boolean) &&
    !env.OIDC_ELEVATION_ISSUER &&
    !env.OIDC_ISSUER
  ) {
    throw new Error(
      "Elevation OIDC requires OIDC_ELEVATION_ISSUER or OIDC_ISSUER",
    );
  }
  if (isProduction && !elevationOidcValues.every(Boolean)) {
    throw new Error("Production requires a dedicated elevation OIDC client");
  }
  ...
  return {
    ...
    oidc: oidcValues.every(Boolean)
      ...
      : null,
    elevationOidc: elevationOidcValues.every(Boolean)
      ? {
          issuer: new URL(env.OIDC_ELEVATION_ISSUER ?? env.OIDC_ISSUER!),
          clientId: env.OIDC_ELEVATION_CLIENT_ID!,
          clientSecret: elevationOidcClientSecret!,
          redirectUri: new URL(env.OIDC_ELEVATION_REDIRECT_URI!),
        }
      : null,
    devAuthBypass,
