...
export class ControlPlaneDatabase {
  ...
  consumePairingCode(
    ...
    return { displayName: row.display_name, mode: row.mode };
  }
    ...
    this.raw
      .prepare(
        ...
        INSERT INTO hosts(
          ...
          kimi_version, capabilities_json, last_seen_at, revoked_at, created_at
  ...
  updateHostStatus(input: {
    ...
    this.raw
      .prepare(
        `
        UPDATE hosts SET state = ?, agent_version = ?, kimi_version = ?, capabilities_json = ?,
          last_seen_at = ? WHERE host_id = ? AND revoked_at IS NULL
      `,
      )
      .run(
        input.state,
    ...
    try {
      ...
      const insert = this.raw.prepare(`
        INSERT INTO session_cache(
          host_id, upstream_session_id, title_enc, workspace_alias_enc, updated_at, state
        ) VALUES (?, ?, ?, ?, ?, ?)
      `);
      for (const session of sessions) {
        ...
        insert.run(
          ...
          this.cipher.encrypt(session.title, `${context}:title`),
  ...
  getSessionCache(hostId: string): SessionCacheItem[] {
    ...
    return rows.map((row) => {
      ...
      return {
        ...
        title: this.cipher.decrypt(row.title_enc, `${context}:title`),
    ...
    this.raw
      .prepare(
        ...
        INSERT INTO audit_events(
          occurred_at, subject, host_id, action_category, outcome, request_id
        ) VALUES (?, ?, ?, ?, ?, ?)
      `,
      )
      .run(
