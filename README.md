...
下表列出当前版本的验证入口和证据边界，不嵌入容易过期的固定测试数量；每次发布应以当前 revision 的本地输出、GitHub CI 和私有部署回执为准；这些检查本身不代表所有生产环境都已完成冷启动验收

<div align="center">

| 检查对象          | 验证方法                | 当前结果                   | 证据边界                               |
| ----------------- | ----------------------- | -------------------------- | -------------------------------------- |
| TypeScript 工作区 | `pnpm test`             | 以当前命令输出为准         | 协议、加密、身份、数据库与界面状态模型 |
| Rust 主机代理     | `cargo test`            | 以当前命令输出为准         | Windows 与 Linux 目标分别验证          |
| 桌面与移动浏览器  | `pnpm test:e2e`         | 以当前命令输出为准         | Chromium 合成数据核心流程              |
| README 合成截图   | `pnpm docs:screenshots` | 以当前命令输出为准         | 桌面亮色、桌面暗色、移动端与配对流程   |
| 上游兼容锁        | `pnpm check:upstream`   | 以当前锁文件和命令输出为准 | Kimi Server API 仍由上游标记为实验性   |
