...
| 能力       | 可观察结果                                                       | 当前状态                            |
| ---------- | ---------------------------------------------------------------- | ----------------------------------- |
| 多主机控制 | 同一界面切换 VPS 与远端 Windows／Linux 主机                      | 已实现，等待公开候选版真实主机验收  |
| Kimi 会话  | 列表、搜索、创建、恢复、归档、分叉、流式事件与断线恢复           | 已实现并通过模拟与本机契约测试      |
...
| 终端       | Windows CMD／PowerShell 与 Linux Shell，支持尺寸、Unicode 和恢复 | 已实现，真实双主机验收尚未完成      |
| 提权终端   | Linux `sudo -k -i` 与独立 Windows LocalSystem Broker             | 已实现，安装与使用需要额外授权      |
...
前置条件：Node.js 24.15.x、pnpm 10.33.x，以及已经安装的 Chromium
...
预期结果：浏览器显示三个合成主机、三条合成会话、权限模式、账户用量和终端入口，不会发起真实控制平面连接
...
推荐部署顺序：身份系统 → TLS 与边缘 → 控制平面 → 主机配对 → Kimi OAuth → 备份与恢复演练 → 真实端到端验收
...
以下数字来自当前 v0.1.0 源码的本地候选版检查，不代表所有生产环境都已验收
...
| 检查对象                  | 验证方法                | 当前结果                           | 证据边界                                 |
| ------------------------- | ----------------------- | ---------------------------------- | ---------------------------------------- |
| TypeScript 协议与控制平面 | `pnpm test`             | 29 个测试通过                      | 模式、加密、身份、数据库、契约与界面模型 |
| Rust 主机代理             | `pnpm check:rust`       | Windows 13 个、Linux 12 个测试通过 | 含 Windows Broker 专用测试               |
| 桌面与移动浏览器          | `pnpm test:e2e`         | 7 个通过，1 个平台条件跳过         | Chromium 合成数据流程                    |
...
项目状态：v0.1.0 预发布，核心实现已完成，真实 Windows 与 VPS 双主机、OAuth、提权和边缘 WAF 仍属于发布阻断验收项
